...
*manage_off_position='<POSITION>'*::
    Position floating windows created by *manage=off*. +
    Accepts the same values as *main_only_float_position* and defaults to 'none'.

*main_only='<BOOL_SEL>'*::
    Keep one tiled main window per application on each space and make additional matching windows float.

*main_only_float_position='<POSITION>'*::
    Position additional floating windows created by *main_only=on*. +
    Valid values are 'none', 'center', 'top_left', 'top_center', 'top_right', 'center_left', 'center_right', 'bottom_left', 'bottom_center', 'bottom_right', and 'fullscreen'. +
    The default value 'none' preserves the position chosen by macOS. 'fullscreen' fills the usable display area while honoring yabai space padding, without entering native fullscreen.
...
*hide='<BOOL_SEL>'*::
    Remove the window from the screen entirely; it becomes invisible and no longer receives mouse events. +
    The window will automatically be treated as a (manage=off) floating window and focus is passed to the next window on the space. +
    Combine with *scratchpad* to bring the window back on demand ('yabai -m window --toggle <LABEL>'), or use 'yabai -m window --scratchpad recover' to forcefully bring back all hidden windows. +
    System Integrity Protection must be partially disabled.
...
----
{
    ...
    "manage-off-position": string,
    "main-only": bool (optional),
    "main-only-float-position": string,
