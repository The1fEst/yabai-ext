...
- Rules support `main_only=on`, allowing one tiled main window per application on each space while keeping additional windows, dialogs, and progress windows floating. `main_only_position` can leave those floating windows in place, center them, anchor them to an edge or corner, or fill the usable display area.
